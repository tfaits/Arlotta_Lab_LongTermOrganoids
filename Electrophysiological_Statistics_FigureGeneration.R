...
df.main$Age <- factor(df.main$Age, levels = c(6, 9, 12, 24))
...
## barplots ----
df.main <- df.main[df.main$Age != "24", ]
df.main <- droplevels(df.main)
...
df.suppl$Age <- factor(df.suppl$Age, levels = c(6, 9, 12, 24))
df.suppl$Measurement <- factor(df.suppl$Measurement, levels = suppl.measures)

## barplots ----
df.suppl <- df.suppl[df.suppl$Age != "24", ]
df.suppl <- droplevels(df.suppl)
